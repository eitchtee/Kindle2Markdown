...
def _parse_clipping_block(lines: List[str]) -> Optional[Dict]:
    ...
    try:
        ...
        highlight = _parse_highlight(lines[3:]) # The highlight starts from the 4th line (index 3)
...
def _parse_title_and_author(line: str) -> Tuple[str, str]:
    """
    Parses the book title and author from the first line of a clipping.
    Handles two cases:
    1. "Book Title (Author)"
    2. "Book Title - Author (Author)"
    """
    line = line.strip()
    ...
    # Check if the title ends with " - Author" and remove it.
...
def _parse_metadata(line: str) -> Tuple[Optional[str], Optional[str], Optional[datetime]]:
    """
